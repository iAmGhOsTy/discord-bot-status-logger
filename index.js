...
client.on('ready', () => {
	...
	const bot = client.users.cache.get(process.env.BOTID);
	const user = client.users.cache.get(process.env.YOURID);
	setInterval(async function() {
		...
		if (!channelExist) {
			const embed = new Discord.MessageEmbed()
				.setColor('#ff0000')
				.setDescription(`Please setup the bot first using ${process.env.PREFIX}setup command.`);
...
client.on('message', async (message) => {
	const bot = client.users.cache.get(process.env.BOTID);
	if (message.content.startsWith(`${process.env.PREFIX}setup`)) {
